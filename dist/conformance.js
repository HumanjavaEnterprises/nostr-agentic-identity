import { CUSTODIES, TIERS } from './types.js';
/**
 ...
 * @returns `{ ok, issues }`
 */
export function isConformant(id) {
    const issues = [];
    // §1 — identity root
    ...
    const tierValid = TIERS.includes(id.tier);
    const custodyValid = CUSTODIES.includes(id.custody);
    if (!tierValid) {
